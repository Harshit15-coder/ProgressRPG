...
- **Payload size as villages/characters scale.** The map endpoint is already scoped per population centre (`/population-centres/<pk>/map/`), so a response only ever carries that one village's residents — growing the *number* of villages doesn't inflate any single poll, and only characters with an active `path` (`is_moving=True`) carry the extra coordinate list at all, not the whole roster. The real growth axis is concurrently-moving characters *within* one village, which is expected to stay a small fraction of residents at once (per your note). Given that, no extra capping/pagination is added now — reuse the existing per-village scoping as the mitigation rather than a new mechanism. If a single village's concurrent-mover count grows enough to matter later, the natural follow-up is capping `path` to the next N nodes (re-requested via the following poll) instead of the full remaining route, but that's not warranted by anything in scope today.
...
- **New backend tests** (`locations/tests.py`): `CharacterPointFeatureSerializer`/`PopulationCentreMapView` returns `path: None` for idle characters and a correct remaining-node coordinate list + `effective_speed` for characters with an active journey.
