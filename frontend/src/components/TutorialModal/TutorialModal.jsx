...
export default function TutorialModal({ onClose, startAtStepId, onComplete }) {
  const { steps, isLoading } = useTutorialSteps();
  const [currentIndex, setCurrentIndex] = useState(() => {
    if (!startAtStepId || !steps.length) return 0;
    const idx = steps.findIndex((s) => s.id === startAtStepId);
    return idx >= 0 ? idx : 0;
  });
  ...
  const handleDone = useCallback(async () => {
    const seenIds = steps.slice(currentIndex).map((s) => s.id);
    try {
