...
function asItems<T>(arr: T[]): AnyListItem[] {
  return arr as unknown as AnyListItem[];
}
...
describe('List', () => {
  ...
  it('renders list items', () => {
    const items = asItems(['Apple', 'Banana', 'Cherry']);
  ...
  it('uses renderItem function when provided', () => {
    const items = [{ name: 'Apple' }, { name: 'Banana' }] as AnyListItem[];
    const renderItem = (item: AnyListItem) => <span>{(item.name as string).toUpperCase()}</span>;
  ...
  it('uses getKey function for generating keys', () => {
    ...
    const getKey = (item: AnyListItem) => item.id as string;

    const { container } = render(
      <List items={items} getKey={getKey} renderItem={(item) => item.name as string} />
    );
  ...
  it('handles items with id property', () => {
    ...
    render(<List items={items} renderItem={(item) => item.name as string} />);
  ...
  it('calls onSelect when selectable item is clicked', async () => {
    ...
    const items = asItems(['Apple', 'Banana']);
  ...
  it('applies selected class to selected item', () => {
    ...
    render(
      <List
        items={items}
        selectedItem={'Apple' as unknown as AnyListItem}
        selectable={true}
      />
    );
  ...
  it('applies custom className to list', () => {
    const { container } = render(
      <List items={asItems(['Apple'])} className="custom-list" />
    );
  ...
  it('applies custom sectionClass to section', () => {
    const { container } = render(
      <List items={asItems(['Apple'])} sectionClass="custom-section" />
    );
  ...
  it('uses getItemClassName for custom item classes', () => {
    const items = asItems(['Red', 'Green', 'Blue']);
    const getItemClassName = (item: AnyListItem) => `color-${String(item).toLowerCase()}`;
  ...
  it('applies hidden class to items with isHidden property', () => {
    ...
    render(
      <List items={items} renderItem={(item) => item.name as string} />
    );
