// SupportFlow/screens/SupportDetailScreen.tsx
...
interface SupportDetailScreenProps {
  supportActionId?: string | null;
  onBackToSupportMenu?: () => void;
}

export default function SupportDetailScreen({
  supportActionId,
  onBackToSupportMenu,
}: SupportDetailScreenProps) {
  const action = supportActionId ? SUPPORT_ACTIONS[supportActionId as keyof typeof SUPPORT_ACTIONS] : undefined;
  // videoEmbedUrl only exists on some entries (e.g. breathing); access via loose cast
  const actionWithVideo = action as (typeof action & { videoEmbedUrl?: string }) | undefined;
  const safeVideoEmbedUrl = getSafeEmbedUrl(actionWithVideo?.videoEmbedUrl);
